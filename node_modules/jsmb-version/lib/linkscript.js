 ...
 * @returns {Array}
 */
function scriptVersion(base, file, options, versionType, versionLen) {
    ...
    var matchList = content.match(/<script[^>]*(>|\/>)/g);
    ...
    matchList.forEach(function (item) {
        // 先将匹配出的<script ... src="xxx" >过滤为src="xxx"
        var tmp = item.match(/src=("|')[^'"]*\.js("|')/);

        if(!tmp){
            return;
        }

        tmp = eval(tmp[0].replace(/src=/g, ''));
        ...
        for(item in cacheList){
            if(fs.existsSync(item)){
                output.push({
                    ...
                    val: cacheList[item].src.replace(cacheList[item].val, cacheList[item].val + '?' + fs.statSync(item).mtime.getTime() / 1000)
                });
            }
        }
    }else{
