 ...
 * @returns {String}
 */
function cmdVersion(base, file, options, versionType, versionLen) {
    ...
    function mapFile(item, curr) {
        // 先判断是否有别名alias
        if (options.alias) {
            item in options.alias ? item = options.alias[item] : null;
        }

        // 再确保模块是以.js后缀
        /\.(js|css)$/.test(item) ? null : item += '.js';
    ...
    if (versionType === 'time') {
        for (item in cacheList) {
            if (fs.existsSync(item)) {
                output.push([cacheList[item], cacheList[item] + '?' + fs.statSync(item).mtime.getTime() / 1000]);
            }
        }
    } else {
        var md5 = require('md5');
