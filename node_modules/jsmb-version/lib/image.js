 ...
 * @param {Array} map           对路径中出现某些需要替换的字符串或正则的map表
 * @param {RegExp} fileType     严格规定需要设置版本号的文件的后缀，如果不属于指定类型，则不做版本号处理
 * @param {String} versionType  版本号的类型，时间戳或md5
 * @param {Number} versionLen   版本号长度
 * @returns {String}
 */
function imageVersion(base, file, map, fileType, versionType, versionLen) {
    fileType ? null : fileType = /\.(tpl|html|htm|css)$/;
    ...
    var matchList = content.match(/<link[^>]*(>|\/>)/g);
    ...
    matchList.forEach(function (item) {
        // 先将匹配出的<link ... href="xxx" >过滤为href="xxx"
        var tmp = item.match(/href=("|')[^'"]*\.css("|')/);

        if(!tmp){
            return;
        }

        tmp = eval(tmp[0].replace(/href=/g, ''));

        // 如果是数组，则需要再循环; 然后递归匹配使用到的模块
        if (tmp instanceof Array) {
            tmp.forEach(function (m) {
                recode(mapFile(m, file), item, m);
            });
        } else {
            recode(mapFile(tmp, file), item, tmp);
        }
    });
    ...
    if(versionType === 'time'){
        for(item in cacheList){
            if(fs.existsSync(item)){
                output.push({
                    src: cacheList[item].src,
                    val: cacheList[item].src.replace(cacheList[item].val, cacheList[item].val + '?' + (fs.statSync(item).mtime.getTime() << 3))
                });
            }
        }
    }else{
        ...
        for(item in cacheList){
            if(fs.existsSync(item)){
                output.push({
                    src: cacheList[item].src,
                    val: cacheList[item].src.replace(cacheList[item].val, cacheList[item].val + '?' + md5(fs.readFileSync(item)).substring(0, versionLen))
                });
            }
        }
    }
