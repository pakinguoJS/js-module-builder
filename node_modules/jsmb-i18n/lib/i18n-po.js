...
I18n.prototype.xgettext = function(src, dist, filename, exist, type, merge) {
	...
	function saveAsPoSingle(file, dest) {
		...
		// 保存
		check ? fs.writeFileSync(dest ? dest : (dist + '/' + filename + type), output, 'utf8') : null;
	}
	...
	function saveAsPoMerge(dir, dest) {
		...
		dest ? mkdir(path.dirname(dest)) : null;

		fs.writeFileSync(dest ? dest : (dist + '/' + 'PO_MERGE_' + (new Date()).getTime() + type), output, 'utf8');
	}
	...
	function saveAsPoMergePro(dir, dest, exist) {
		...
		// 已存在的
		var existedList = fs.readFileSync(exist, 'utf8').match(/#:\s[^\n]*(\r|\n|\r\n)msgid (""((\r|\n|\r\n)"[^\n]*")+|"[^\n]*")(\r|\n|\r\n)msgstr (""((\r|\n|\r\n)"[^\n]*")+|"[^\n]*")/g);
	...
	function extract(file, merge) {
		...
		function logRepeat() {
			if (JSON.stringify(repeat).length > 2) {
				...
				fs.writeFileSync(dist + '/' + 'PO_MERGE_REPEAT.log', log, 'utf8');
			}
		}
	}
}
...
I18n.prototype.gettext = function(lang, i18n, src, dist, replacestr) {
	...
	function gettextPo() {
		// 读取文件修改记录的缓存文件
		var recode = path.join(process.cwd(), '__buildjs', ('I18N_' + lang).toUpperCase() + '.json');
		...
		// 获取翻译文件
		var matches = fs.readFileSync(i18n, 'utf8').match(/msgid (""((\r|\n|\r\n)"[^\n]*")+|"[^\n]*")(\r|\n|\r\n)msgstr (""((\r|\n|\r\n)"[^\n]*")+|"[^\n]*")/g);
			...
			list.forEach(function(name, idx) {
				...
				if (fs.statSync(file).isDirectory()) {
					...
					if (!(file in cache && cache[file] === time)) {
						cache[file] = time;
						if (_this._fileType.test(path.extname(file))) {
							// console.log(file);
