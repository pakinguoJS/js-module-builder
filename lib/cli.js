...
function inotifyRsync(){
    var argvs = process.argv;
    require('./task/inotifywait-start.js')(jsmb.config.src, argvs.length > 4 ? jsmb.util.path(argvs[3]) : jsmb.util.path('./_jsmb_/jsmb.conf.js'), jsmb.config.ignore);
}


function inotifyStop(){
    require('./task/inotifywait-stop.js')(jsmb.config.src);
}
...
/**
 * 若未使用实时同步功能，则需要在release前增加一步遍历有新增或修改的文件
 */
function doSync(){
...
============================================================================================\n\
Usage A:\n\
  jsmb iwatch         Linux下开启后台实时文件监听（inotifywait方式）\n\
  jsmb istop          Linux下关闭后台实时监听进（inotifywait方式）\n\
  jsmb-start          Linux下开启后台进程实时文件监听（chokidar方式）\n\
  jsmb-stop           Linux下关闭后台实时监听进程(chokidar方式)\n\
--------------------------------------------------------------------------------------------\n\
Usage B: jsmb [type]\n\
  [Options]             [Description]\n\
    init                初始化配置文件jsmb.conf.js，默认保存在当前路径下的_jsmb_/jsmb.conf.js\n\
    watch [confPath]    对当前路径开启实时文件监听（Linux下建议使用jsmb iwatch 或 jsmb-start）\n\
