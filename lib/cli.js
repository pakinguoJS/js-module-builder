...
function initConfig(configpath) {
    ...
    jsmb.config.uglifyjs.cwd = go(jsmb.config.uglifyjs.cwd);
    jsmb.config.minifycss.cwd = go(jsmb.config.minifycss.cwd);
}
...
function rsync() {
    ...
    }, jsmb.util.path('_jsmb_/rsync.time'));
...
function release() {
    ...
    uglifyjs();
    minifycss();
    console.log("[OK]".green + " ... uglifyjs & minifycss".yellow);
...
Usage: jsmb [type]\n\
  [options]             [description]\n\
    init                初始化jsmb.conf.js文件，默认将文件保存在当前路径下的_jsmb_/jsmb.conf.js\n\
    jsmb-start          Linux下开启后台进程实时文件监听（同步文件）\n\
    jsmb-stop           Linux下关闭后台实时监听进程\n\
    watch               对当前路径开启实时文件监听（Linux下建议使用jsmb-start）\n\
    xgettext            提取待翻译的字段，[lang]支持多语言，不同语言间用", "隔开\n\
    gettext             根据指定语言类型翻译已提取的字段，[lang]同xgettext\n\
    release             根据配置文件制定的编译功能进行全局编译\n\
    clean               清除制定的文件（夹）\n\
============================================================================================"
    );
}
